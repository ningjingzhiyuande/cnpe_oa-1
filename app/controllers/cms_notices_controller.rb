class CmsNoticesController < ApplicationController
   before_action :set_cms_article, only: [:show, :edit, :update, :destroy]
  skip_before_filter :verify_authenticity_token, :only => [:destroy]

  def index
    @cms_articles = CmsArticle.notices
  ...
  def create
    ...
    @cms_article.save
    respond_with(@cms_article,location: cms_notice_path(@cms_article))
  end
  ...
  def destroy
    @cms_article.destroy
    respond_with(@cms_article,location: cms_notices_path)
  end
